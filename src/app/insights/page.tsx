 ...
 * 3) Renders charts/cards
 * 4) Calls your Next API route (Dedalus-backed) to generate an AI explanation
 *
 * IMPORTANT:
...
function mergeDaily(daily: DailyAgg[], watch: WatchDailyRow[], vids: VideoSentimentRow[]): MergedRow[] {
    ...
    async function load() {
      ...
      try {
        const [tx, watch, vids] = await Promise.all([
          fetch('/mock/transactions.json').then((r) => r.json() as Promise<TxRow[]>),
          fetch('/mock/watch_daily.json').then((r) => r.json() as Promise<WatchDailyRow[]>),
          fetch('/mock/video_sentiment.json').then((r) => r.json() as Promise<VideoSentimentRow[]>),
        ]);
  ...
  async function requestAiSummary(reason: 'auto' | 'manual' = 'manual') {
    ...
    try {
      ...
      const res = await fetch(SUMMARY_API, {
        ...
        body: JSON.stringify({
          // give the server a clear task prompt
          prompt:
            'You are a helpful personal analytics coach. Explain the patterns found in the user’s data. ' +
            'Call out what happened on unusually high wants-spending days. ' +
            'Provide 3–6 concrete suggestions the user can try next week. ' +
            'Avoid medical advice. Keep it concise and actionable.',
          data: payload,
          meta: { source: 'insights-page', trigger: reason },
        }),
      });

      const ct = res.headers.get('content-type') || '';
      if (!res.ok) {
        // If server returned HTML (like Next notFound page), show a useful error.
